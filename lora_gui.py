...
def train_model_alt(
    ...
    run_cmd += f' --dataset_config="{toml_config_path}"'
    run_cmd += f' --discriminator_config_path="{discriminator_config_path}"'
...
def lora_tab(
    ...
    with gr.Tab("Training"):
        ...
        with gr.Row():
            toml_config_path = gr.Textbox(
                label="TOML Config Path",
                placeholder=r"H:\path\to\config.toml",
                info="Specify the TOML config Path.",
            )
            discriminator_config_path = gr.Textbox(
                label="Discriminator Config Path",
                placeholder=r"H:\path\to\discriminator_cofig.json",
                info="Specify the discriminator_cofig Path.",
            )
        button_run_alt = gr.Button("Start Custom Training", variant="primary")
        ...
        button_run_alt.click(
            train_model_alt,
            inputs=[dummy_headless] + [dummy_db_false] + settings_list + [toml_config_path, discriminator_config_path] ,
            show_progress=False,
        )
